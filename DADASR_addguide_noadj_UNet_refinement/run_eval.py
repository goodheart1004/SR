...
def parse_args():
    ...
    boundary_group.add_argument(
        "--boundary-refinement",
        dest="boundary_refinement",
        action="store_true",
        default=None,
        help="Override args.csv and enable boundary-aware dual residual heads.",
    )
    boundary_group.add_argument(
        ...
        action="store_false",
        default=None,
        help="Override args.csv and use the original single residual head.",
    )
    semantic_group = parser.add_mutually_exclusive_group()
    semantic_group.add_argument(
        "--semantic-modulation",
        dest="semantic_modulation",
        action="store_true",
        default=None,
        help="Override args.csv and enable SAM3 semantic modulation in local refinement.",
    )
    semantic_group.add_argument(
        "--no-semantic-modulation",
        dest="semantic_modulation",
        action="store_false",
        default=None,
        help="Override args.csv and disable SAM3 semantic modulation.",
    )
    parser.add_argument(
        "--semantic-channels",
        type=int,
        default=None,
        help="Override the SAM3 semantic encoder channel count loaded from args.csv.",
    )
    ...
    cross_modal_group = parser.add_mutually_exclusive_group()
    cross_modal_group.add_argument(
        "--cross-modal-fusion",
        dest="use_cross_modal_fusion",
        action="store_true",
        default=None,
        help="Override args.csv and enable explicit RGB--LR DSM fusion before refinement.",
    )
    cross_modal_group.add_argument(
        ...
        action="store_false",
        default=None,
        help="Override args.csv and disable explicit RGB--LR DSM fusion.",
    )
    parser.add_argument(
        "--cross-modal-reduction",
        type=int,
        default=None,
        help="Override the joint RGB--DSM gate channel reduction ratio loaded from args.csv.",
    )
    parser.add_argument("--num-heatmaps", type=int, default=10, help="Number of evaluated samples to save as heatmaps")
    parser.add_argument(
        "--num-fusion-visuals",
        type=int,
        default=1,
...
def load_train_args(checkpoint_path, cli_args):
    ...
    if os.path.isfile(args_csv_path):
        ...
        if "boundary_refinement" not in loaded:
            values["boundary_refinement"] = False
        if "semantic_modulation" not in loaded:
            values["semantic_modulation"] = False
        if "semantic_channels" not in loaded:
            values["semantic_channels"] = 32
...
def build_model(train_args, device):
    model = GADBase(
        ...
        boundary_refinement=getattr(train_args, "boundary_refinement", False),
        semantic_modulation=getattr(train_args, "semantic_modulation", False),
        semantic_channels=getattr(train_args, "semantic_channels", 32),
        use_cross_modal_fusion=getattr(train_args, "use_cross_modal_fusion", False),
        cross_modal_reduction=getattr(train_args, "cross_modal_reduction", 2),
    )
...
def main():
    ...
    print(
        ...
        f"boundary_refinement={train_args.boundary_refinement}, "
        f"semantic_modulation={train_args.semantic_modulation}, "
        f"semantic_channels={train_args.semantic_channels}, "
        f"refinement_only={train_args.refinement_only}, "
        f"use_cross_modal_fusion={train_args.use_cross_modal_fusion}, "
        f"cross_modal_reduction={train_args.cross_modal_reduction}"
    )
