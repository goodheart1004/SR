...
def download_ckpt_from_hf(
    local_ckpt_path: str = "./sam3ckpt/sam3.pt",
    allow_hf_download: bool = False,
):
    """Resolve a SAM3 checkpoint path.

    Default behavior is offline-first:
    - If a local checkpoint exists at `local_ckpt_path`, use it.
    - If not found and `allow_hf_download` is True, fall back to Hugging Face Hub.
    - Otherwise raise FileNotFoundError.

    This is intentionally used by the builder functions when `load_from_HF=True`
    and `checkpoint_path` is None.
    """
    # 1) Local path relative to current working directory.
    cand1 = Path(local_ckpt_path).expanduser()

    # 2) Also try relative to this file's directory, in case cwd differs.
    cand2 = (Path(__file__).resolve().parent / local_ckpt_path).expanduser()
    ...
    if not allow_hf_download:
        raise FileNotFoundError(
            f"SAM3 checkpoint not found. Expected a local file at '{cand1}' (or '{cand2}'). "
            f"Place 'sam3.pt' there, or pass checkpoint_path explicitly, or set allow_hf_download=True."
        )
...
def build_sam3_video_model(
    ...
    # Create main dot product scorin
