...
parser.add_argument('--boundary-refinement', dest='boundary_refinement', action='store_true', default=True, help='Use boundary-aware dual residual heads in the local refinement network')
parser.add_argument('--no-boundary-refinement', dest='boundary_refinement', action='store_false', help='Use the original single residual head in the local refinement network')
semantic_group = parser.add_mutually_exclusive_group()
semantic_group.add_argument('--semantic-modulation', dest='semantic_modulation', action='store_true', default=False, help='Modulate local refinement residual blocks with SAM3 building/road semantic features')
semantic_group.add_argument('--no-semantic-modulation', dest='semantic_modulation', action='store_false', help='Disable SAM3 semantic modulation (default; preserves legacy checkpoints)')
parser.add_argument('--semantic-channels', type=int, default=32, help='Hidden channel count in the SAM3 semantic feature encoder')
parser.add_argument('--cross-modal-fusion', dest='use_cross_modal_fusion', action='store_true', default=True, help='Fuse explicit RGB--LR DSM local similarity and joint-statistics gates before refinement')
parser.add_argument('--no-cross-modal-fusion', dest='use_cross_modal_fusion', action='store_false', help='Disable explicit RGB--LR DSM feature fusion')
parser.add_argument('--cross-modal-reduction', type=int, default=2, help='Channel reduction ratio for joint RGB--DSM gating')
