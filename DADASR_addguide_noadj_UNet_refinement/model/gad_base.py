...
class SemanticFeatureEncoder(nn.Module):
    """Encode building/road masks and their valid-region mask for modulation."""

    def __init__(self, semantic_channels):
        super().__init__()
        if semantic_channels <= 0:
            raise ValueError('semantic_channels must be positive when semantic modulation is enabled')
    ...
    def forward(self, semantic_masks, semantic_valid_mask):
        # The valid mask preserves the distinction between class 0 (no prompt)
        # and ignored/unknown pixels, where modulation is suppressed.
        inputs = torch.cat(
...
class SemanticFiLMResidualBlock(nn.Module):
    """Residual block whose activations are spatially FiLM/gate modulated."""
...
class DownBlock(nn.Module):
    ...
    def forward(self, x):
        return self.residual_blocks(self.downsample(x))
...
class UpBlock(nn.Module):
    ...
    def __init__(self, channels):
        super().__init__()
        self.projection = ConvRelu(2 * channels, channels)
...
class LocalRefinementNet(nn.Module):
    ...
    def __init__(
        ...
        self.semantic_modulation = semantic_modulation
    ...
    def forward(
        ...
        if self.semantic_modulation:
            semantic_masks, semantic_valid_mask = self._resize_semantic_inputs(
                semantic_masks,
                semantic_valid_mask,
                x,
            )
            ...
            if semantic_gates:
                semantic_block_gates = torch.stack(semantic_gates, dim=1)
            else:
                semantic_block_gates = x.new_zeros(x.shape[0], 0, x.shape[1], x.shape[2], x.shape[3])
            semantic_aux = {
                'semantic_features': semantic_features,
                'semantic_masks': semantic_masks,
                'semantic_valid_mask': semantic_valid_mask,
                'semantic_block_gates': semantic_block_gates,
            }
        else:
    ...
    @staticmethod
    def _resize_semantic_inputs(semantic_masks, semantic_valid_mask, reference):
        if semantic_masks is None or semantic_valid_mask is None:
            raise ValueError(
                'semantic_modulation=True requires semantic_masks [B, 2, H, W] '
                'and semantic_valid_mask [B, 1, H, W].'
            )
        if semantic_masks.ndim != 4 or semantic_masks.shape[1] != 2:
            raise ValueError(
                'semantic_masks must have shape [B, 2, H, W] for building and road; '
                f'got {tuple(semantic_masks.shape)}.'
            )
        if semantic_valid_mask.ndim == 3:
            semantic_valid_mask = semantic_valid_mask.unsqueeze(1)
        if semantic_valid_mask.ndim != 4 or semantic_valid_mask.shape[1] != 1:
            raise ValueError(
                'semantic_valid_mask must have shape [B, 1, H, W]; '
                f'got {tuple(semantic_valid_mask.shape)}.'
            )
        if semantic_masks.shape[0] != reference.shape[0] or semantic_valid_mask.shape[0] != reference.shape[0]:
            raise ValueError(
                'Semantic inputs and refinement features must use the same batch size: '
                f'{semantic_masks.shape[0]}, {semantic_valid_mask.shape[0]}, and {reference.shape[0]}.'
            )
        ...
        if semantic_masks.shape[-2:] != reference.shape[-2:]:
            semantic_masks = F.interpolate(semantic_masks, size=reference.shape[-2:], mode='nearest')
        if semantic_valid_mask.shape[-2:] != reference.shape[-2:]:
            semantic_valid_mask = F.interpolate(semantic_valid_mask, size=reference.shape[-2:], mode='nearest')
        return semantic_masks.clamp(0.0, 1.0), semantic_valid_mask.clamp(0.0, 1.0)
...
class GADBase(nn.Module):
    ...
    def forward(self, sample, train=False):
        ...
        if self.refinement_net is not None:
            ...
            y_init, refinement_residual, refinement_aux = self.refinement_net(
                guide_feats,
                y_bicubic,
                boundary_prior=refinement_boundary_prior,
                semantic_masks=semantic_masks,
                semantic_valid_mask=semantic_valid_mask,
            )
    ...
    @staticmethod
    def _validate_semantic_sample(sample):
        schema_valid = sample.get('semantic_schema_valid')
        if schema_valid is None:
            raise ValueError(
                'semantic_modulation=True requires semantic_schema_valid from ProcessedDSMDataset. '
                'Expected labels use only 0 (no prompt), 1 (building), 2 (road), or 255 (ignore).'
            )
        if not torch.is_tensor(schema_valid):
            schema_valid = torch.as_tensor(schema_valid, dtype=torch.bool)
        if not schema_valid.bool().all().item():
            raise ValueError(
                'semantic_modulation=True requires SAM3 labels with schema '
                '0=no prompt, 1=building, 2=road, 255=ignore. '
                'At least one sample has an incompatible label (for example legacy class 3); '
                'do not enable semantic modulation until those labels are converted.'
            )
        missing = [
        ...
        if missing:
            raise ValueError(
                'semantic_modulation=True requires semantic dataset fields: '
                + ', '.join(missing)
            )
