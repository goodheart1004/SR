...
# SAM3 semantic-label schema used by semantic modulation.  Keep this separate
# from ``boundary_prior``, which preserves the original visual boundary path.
...
class ProcessedDSMDataset(Dataset):
    ...
    @staticmethod
    def _make_boundary_prior(sample):
        if 'sam3' in sample:
            source = sample['sam3']
    ...
    @staticmethod
    def _make_semantic_inputs(sample):
        """Build SAM3 semantic modulation inputs without changing legacy priors.

        ``label`` is deliberately not coerced into another schema here.  A
        sample that contains labels other than 0/1/2/255 is marked invalid and
        is rejected by ``GADBase`` only when semantic modulation is enabled.
        This keeps legacy training and checkpoints usable while preventing an
        accidental semantic run on old label maps.
        """
        target_size = sample['y'].shape[-2:]
        ...
        if label.ndim != 3 or label.shape[0] != 1:
            raise ValueError(
                'Semantic labels must have shape [1, H, W] before batching; '
        ...
        semantic_boundary_prior = ProcessedDSMDataset._semantic_boundary_map(
            label,
            semantic_valid_mask,
        )
        return semantic_masks, semantic_valid_mask, schema_valid, semantic_boundary_prior

    @staticmethod
    def _semantic_boundary_map(label, valid_mask):
        """Return a dilated categorical boundary map for 0/building/road.

        Ignore pixels do not create category boundaries, so they cannot route
        the edge residual branch merely because an annotation is missing.
        """
    ...
    @staticmethod
    def _edge_map(tensor):
        tensor = torch.nan_to_num(tensor.float(), nan=0.0, posinf=0.0, neginf=0.0)
        if tensor.ndim == 2:
            tensor = tensor.unsqueeze(0)

        _, height, width = tensor.shape
        dx = tensor.new_zeros(1, height, width)
        dy = tensor.new_zeros(1, height, width)
        if width > 1:
