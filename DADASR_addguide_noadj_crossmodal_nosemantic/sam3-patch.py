...
SEMANTIC_CLASSES = (
    ...
    ("tree", "tree, forest canopy, shrub"),
    ("ground_vegetation", "bare ground, grassland, agricultural field"),
    ("water", "water body"),
)
...
def main():
    ...
    ap.add_argument("--prompts", nargs="+", default=DEFAULT_PROMPTS,
                    help="固定顺序: building road tree ground_vegetation water；顺序决定 label ID")
    ap.add_argument(
        "--class_names",
        nargs="+",
        default=DEFAULT_CLASS_NAMES,
        help="与 --prompts 一一对应的稳定类别名；默认: building road tree greenland water",
    )
    ...
    if args.road_prompt is not None or args.building_prompt is not None:
        args.prompts = [
            args.road_prompt or "street,asphalt road,driveway,pavement",
            args.building_prompt or "building",
        ]
    if len(args.prompts) != len(SEMANTIC_CLASSES):
        ap.error(
            "Semantic-modulation labels require exactly five prompts in this order: "
            "building road tree ground_vegetation water. "
            "Do not use the legacy two-prompt arguments for this dataset."
        )
